...
order_url = 'https://www.gestion.nicaraguacompra.gob.ni/siscae/portal/adquisiciones-gestion/busqueda/__ac0x3adquisiciones-gestion-portlet0x2busquedaProcedimientoPortlet?&org.apache.myfaces.portlet.MyFacesGenericPortlet.VIEW_ID=/fragments/busqueda/datos/datosProcedimientoPublicoView.jsp&datosProcedimiento%3APluto__adquisiciones_gestion_portlet_busquedaProcedimientoPortlet__id23=Adjudicaci%F3n&datosProcedimiento_SUBMIT=1&autoScroll=0%2C142&datosProcedimiento%3A_link_hidden_='
...
def get_orders(url,refer):
    ...
    try:
        ...
        text = soup.getText()
        i=0
        while soup.find(id="datosProcedimiento:datosAdjudicacionesDGCEView:datosAdjudicacionesDGCEId") == None:
            print(f"\n{i} : Running loop")
            time.sleep(2)
            response = session.get(url, headers={'Referer': refer})
            soup = BeautifulSoup(response.text, 'html.parser')
            ...
            if i ==3:
                break
        if soup.find(id="datosProcedimiento:datosAdjudicacionesDGCEView:listadoAdjudicaciones") != None:
            result_text = soup.find(id="datosProcedimiento:datosAdjudicacionesDGCEView:listadoAdjudicaciones_0:proveedorNombreColV").string if soup.find(id="datosProcedimiento:datosAdjudicacionesDGCEView:listadoAdjudicaciones_0:proveedorNombreColV") != None else soup.find(id="datosProcedimiento:datosAdjudicacionesDGCEView:listadoAdjudicaciones_0:proveedorRUCColV").string
            print(f"\nFound: {result_text}")
            return response
        else:
...
def get_scraper(results,datos):
    ...
    try:
        ...
        scraped_data = scraping_mti_adj2.process_resultpage(results,datos)
    except Exception as e:
...
print(f'\nNumber of pages iterated: {math.ceil(int(number_of_results[0])/100)}')

datos_result = []
dato =0
for x in range(0,range_number):
    print(f'\n Getting datos for {x}')
    p_name = soup.find(id="resultadoView:listadoProcedimientosForm:listadoProcedimientos_"+str(x)+":Pluto__adquisiciones_gestion_portlet_busquedaProcedimientoPortlet__id19").string
    p_nm = soup.find(id="resultadoView:listadoProcedimientosForm:listadoProcedimientos_"+str(x)+":Pluto__adquisiciones_gestion_portlet_busquedaProcedimientoPortlet__id20").string
    print(f'For: {p_name} & {p_nm}')
...
if math.ceil(int(number_of_results[0])/100) > 2:
    for i in range(1,math.ceil(int(number_of_results[0])/100)):
        try:
            print(f"\n========================Inside page_loop: {i}========================")
            url2 = second_page_url+str(i)+second_page_last
            print(f"\nSecond Page URL: {url2}")
            second_page_result = get_data(url2, second_page_refer)
            second_page_result = get_data(url2, second_page_refer)
            ...
            print(f"Error in page_loop: {e}")
else:
    print(f"\nSecond Page URL: {second_page_url}")
    url2 = second_page_url+"1"+second_page_last
    second_page_result = get_data(url2, second_page_refer)
    soup = BeautifulSoup(second_page_result.text, 'html.parser')
    total_results = soup.find(id="resultadoView:listadoProcedimientosForm:listadoProcedimientos:tbody_element")
    count = 0
