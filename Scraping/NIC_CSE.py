...
Purpose: File to scrape and match all information for each contract
"""
...
def get_scraper(results,datos):
    ...
    try:
        ...
        scraped_data = scraping_cse.process_resultpage(results,datos)
    except Exception as e:
...
results = get_data(trialurl, refer) #this methods fetches the main_page results
...
#this section scrapes the mas_datos section to get more information
datos_result = []
dato =0
...
#this function calls another file which maps the website information to the ERD data
result_scraped = get_scraper(next_page_result_1, datos_result)

#this section iterates through next pages and loops over to fetch all information from the ministry
